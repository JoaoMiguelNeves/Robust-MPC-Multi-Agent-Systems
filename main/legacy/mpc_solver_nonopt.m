function [x, t_collision] = mpc_solver_nonopt(n, h, tmax, m, detect_collisions, obstacleVertices)
% Solves the optimization problem using the mpc method
	...
	for iter = 1:tmax
		...
		Bconsfunction [x, t_collision] = mpc_solver(n, h, tmax, m, detect_collisions, obstacleVertices)
% Solves the optimization problem using the mpc method

	Ai = kron([1 1;0 1], eye(2)); % double integrator ith node dynamics
	Bi = krontraint = [zeros(nx,nu*h); kron(eye(h),B)];
